...
public interface Store_Order_Read_Mapper {
	...
	@Select(" SELECT IFNULL(SUM(MENU_PRICE), 0) FROM STORE_ORDER WHERE REF = #{ref} AND DELFLAG = 'N' AND PAYED = 'N' ")
	...
	@Select(" SELECT MENU_SEQ menu_seq, MENU_NAME menu_name,\n" +
		"       COUNT(*)  count, (SUM(MENU_PRICE)/COUNT(*)) price,SUM(MENU_PRICE) sum, MIN(ORDER_DATE) order_date\n" +
		...
		"GROUP BY SO.MENU_SEQ ORDER BY order_date ")
	List<HashMap<String, Object>> getTableOrder(@Param("table_seq") int table_seq, @Param("store_seq") int store_seq);
	
	@Select(" SELECT IFNULL(MAX(REF), (SELECT MAX(REF)+1 ref FROM STORE_ORDER)) ref FROM STORE_ORDER WHERE STORE_SEQ = #{store_seq} AND TABLE_SEQ = #{table_seq} AND DELFLAG ='N' AND PAYED = 'N' ")
